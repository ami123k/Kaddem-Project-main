...
public class IUniversiteServiceImpl extends IGenericServiceImp<Universite,Integer> implements IUniversiteService{
   ...
   // @Scheduled(cron = "*/20 * * * * ?")
    public void test(){
//        this.contratRepository.findByArchiveIsFalse().stream()
        /*        .map(Universite::getDepartements)
                .flatMap(Collection::stream)*/
//                .forEach(contrat -> log.info(""+contrat.getEtudiant().getEquipes().size()));
        List<Universite> universites = this.retrieveAll();
        for (Universite universite: universites) {
           universite.getDepartements().stream()
                   .forEach(departement -> log.info(""+departement.getNomDepart()))
                                   ;
//            log.info(" " +nb);
        }
    }
